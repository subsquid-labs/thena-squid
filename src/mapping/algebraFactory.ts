...
export function getAlgebraFactoryActions(ctx: MappingContext<StoreWithCache>, item: Item) {
    if (item.address !== ALGEBRA_FACTORY) return
...
function createPoolHandler(ctx: MappingContext<StoreWithCache>, log: Log) {
    const callCache = CallCache.get(ctx)

    const event = algebraFactory.events.Pool.decode(log)
